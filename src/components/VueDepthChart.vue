...
const defaultDayOptions = {
  ...
  // 轴配置
  xAxisGridSpace: 50, // x轴背景网格间距
  yAxisGridSpace: 20, // y轴背景网格间距
...
export default {
  ...
  methods: {
    ...
    drawAxis (data, context, options) {
      ...
      const buyData = buyOriginData.sort(function (a, b) { return b.price - a.price })
      const sellOriginData = allData.sell
      const sellData = sellOriginData.sort(function (a, b) { return b.price - a.price })
      ...
      const buyMaxAmount = Math.max(...buyData.map(o => o.amount))
      const sellMaxAmount = Math.max(...sellData.map(o => o.amount))
      const totalAmount = 0.5 // Math.max(buyMaxAmount, sellMaxAmount) // 确定最大高度对应的数量
      ...
      for (
        ...
        // context.moveTo(origin[0] + 25, j)
        // context.lineTo(origin[0] + 29, j)
        ...
        context.fillText(number, origin[0] + 1, j + 6)
      }
    ...
    deepClone (obj) {
      ...
      const newObj = new obj.constructor() // 如果obj是数组，newObj=[];如果obj是对象，newObj={}
